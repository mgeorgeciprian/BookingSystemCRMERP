...
export default function ServicesPage() {
  ...
  const filtered = activeCategory
    ? MOCK_SERVICES.filter((s) => s.category_id === activeCategory)
    : MOCK_SERVICES;
    ...
    <div className="p-6 lg:p-8">
      <div className="mb-8 flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500">{MOCK_SERVICES.length} servicii in {MOCK_CATEGORIES.length} categorii</p>
        </div>
      ...
      <div className="mb-6 flex gap-2 flex-wrap">
        ...
        >
          Toate ({MOCK_SERVICES.length})
        </button>
        {MOCK_CATEGORIES.map((cat) => {
          const count = MOCK_SERVICES.filter((s) => s.category_id === cat.id).length;
          return (
            <button
              key={cat.id}
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {filtered.map((svc) => {
          const cat = MOCK_CATEGORIES.find((c) => c.id === svc.category_id);
          return (
            <div
              key={svc.id}
            ...
            >
              ...
              <div className="mt-4 flex items-center justify-between">
                <div>
                  ...
                  <span className="text-sm text-gray-400"> {svc.currency}</span>
                </div>
              ...
              <div className="mt-3 flex items-center justify-between border-t pt-3">
                <span className="text-[10px] text-gray-400">
                  {cat?.name} &middot; TVA {svc.vat_rate}%
                </span>
